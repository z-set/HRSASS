...
<script>
import { getPeopleJob, updatePeopleJob, getPeopleSimple } from '@/api/employess'
...
export default {
  ...
  created() {
    this.getPeopleJob()
    this.getPeopleSimple()
  },
  methods: {
    // saveJob 提交保存
    async saveJob() {
      await updatePeopleJob(this.formData)
      this.$message.success('操作成功')
    },
    // 获取员工岗位信息
    async getPeopleJob() {
      this.formData = await getPeopleJob(this.userId)
    },
    // 获取员工简单列表
    async getPeopleSimple() {
      this.depts = await getPeopleSimple()
    }
  }
}
</script>
