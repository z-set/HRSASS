...
<script>
import { getUserSimple, updateUserSimple, updateUserPersonal, getUserPersonal } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      // 发请求
      const fileList = this.$refs.img1.fileList
      // 判断当前图片组件上传是否完成
      if (fileList.some(item => item.upload !== true)) {
        this.$message.error('图片上传中，请稍后')
        return
      }
      //   当没有上传文件时 返回一个' ' 字符串给后台
      await updateUserSimple({ ...this.userInfo, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' })
      this.$message.success('操作成功')
    },
    // savePersonal保存更新
    async savePersonal() {
      const fileList = this.$refs.img2.fileList
      // 判断当前图片组件上传是否完成
      if (fileList.some(item => item.upload !== true)) {
        this.$message.error('图片上传中，请稍后')
        return
      ...
      //   当没有上传文件时 返回一个' ' 字符串给后台
      await updateUserPersonal({ ...this.formData, id: this.userId, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' })
      this.$message.success('操作成功')
    },
    // 获取员工基本信息
    async getUserSimple() {
      this.userInfo = await getUserSimple(this.userId)
      // 判断数据中有没有头像 有就在组件中显示 空字符也排除在外
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        this.$refs.img1.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    },
    // 获取员工个人信息
    async getUserPersonal() {
      this.formData = await getUserPersonal(this.userId)
      //   判断数据中有没有头像 有就在组件中显示 空字符也排除在外
      if (this.formData.staffPhoto && this.formData.staffPhoto.trim) {
        this.$refs.img2.fileList = [{ url: this.formData.staffPhoto, upload: true }]
      }
    }
  }
}
