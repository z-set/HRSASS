...
<script>
import { getPeopleJob } from '@/api/employess'
import { getUserSimple, getUserPersonal } from '@/api/user'
export default {
  data() {
    return {
      formData: {},
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getUserPersonal(this.userId) // 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await getUserSimple(this.userId)
      const jobInfo = await getPeopleJob(this.userId) // 获取个人基本信息
